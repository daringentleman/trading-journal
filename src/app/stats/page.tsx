    ...
    <div className="min-h-screen px-4 py-5 md:px-8 md:py-7">
      ...
      <div className="md:flex md:items-center md:gap-4 md:mb-5">
        ...
        <div className="flex items-center gap-2 mb-4 md:mb-0">
          <button onClick={() => setMonth(p => { const d = new Date(p.y, p.m - 1); return { y: d.getFullYear(), m: d.getMonth() } })}
            className="w-8 h-8 retro-card flex items-center justify-center fs-stat font-bold">‹</button>
          <span className="retro-display fs-stat px-3 py-1.5 retro-card min-w-[110px] text-center">{monthLabel}</span>
          <button onClick={() => setMonth(p => { const d = new Date(p.y, p.m + 1); return { y: d.getFullYear(), m: d.getMonth() } })}
            className="w-8 h-8 retro-card flex items-center justify-center fs-stat font-bold">›</button>
        </div>
        ...
        <div className="hidden md:grid grid-cols-3 gap-2 flex-1">
          {[
            ...
            <div key={s.label} className="retro-card px-4 py-2.5 flex items-center justify-between gap-3">
              <span className="fs-tiny font-bold" style={{ color: 'var(--muted)' }}>{s.label}</span>
              <span className="retro-display retro-mono fs-stat-big" style={{ color: s.color }}>{s.val}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="grid grid-cols-3 gap-2 mb-4 md:hidden">
        {[
          ...
          <div key={s.label} className="retro-card p-3">
            <div className="fs-tiny font-bold mb-1" style={{ color: 'var(--muted)' }}>{s.label}</div>
            <div className="retro-display retro-mono fs-stat-big" style={{ color: s.color }}>{s.val}</div>
          </div>
        ))}
      </div>
      ...
      <section className="retro-card mb-5 overflow-hidden">
        <div className="px-4 py-3 flex items-center justify-between"
          ...
          <h2 className="retro-display fs-section" style={{ color: 'var(--border)' }}>資產走勢</h2>
          <span className="fs-meta retro-display" style={{ color: 'var(--border)' }}>EQUITY · CHART</span>
        </div>
        <div className="p-4">
      ...
      <section className="retro-card mb-5 overflow-hidden">
        <div className="px-4 py-3 flex items-center justify-between"
          style={{ background: 'var(--accent)', borderBottom: '1.5px solid var(--border)' }}>
          <h2 className="retro-display fs-section" style={{ color: 'var(--border)' }}>策略總覽</h2>
          <span className="fs-meta retro-display" style={{ color: 'var(--border)' }}>STRATEGY · OVERVIEW</span>
        </div>
        ...
        <div className="p-4">
          {strategyMetrics.length > 0 ? (
            ...
            <div className="p-8 text-center fs-meta font-bold" style={{ color: 'var(--muted)' }}>
              此時間範圍內無策略資料
            </div>
          )}
        </div>
      ...
      {monthlyData.length > 0 && (
        <section className="retro-card overflow-hidden">
          <div className="px-4 py-3 flex items-center justify-between"
            style={{ background: 'var(--accent)', borderBottom: '1.5px solid var(--border)' }}>
            <h2 className="retro-display fs-section" style={{ color: 'var(--border)' }}>月度損益追蹤</h2>
            <span className="fs-meta retro-display" style={{ color: 'var(--border)' }}>MONTHLY · LEDGER</span>
          </div>
          <table className="w-full border-collapse retro-mono fs-body">
            <thead>
              <tr style={{ background: 'var(--raised)', borderBottom: '1.5px solid var(--border)' }}>
                {['月份', '起始', '損益', '結餘'].map(h => (
                  <th key={h} className="text-left py-2 px-3 fs-tiny font-bold" style={{ color: 'var(--border)' }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
